...
namespace Multiformats.Base
{
    internal class Base64Url : Base64
    {
        ...
        public override ReadOnlySpan<byte> Decode(ReadOnlySpan<char> input) => Decode(input, true, false);
        public override string Encode(byte[] bytes) => Encode(bytes.AsSpan(), true, false).ToString();
        public override ReadOnlySpan<char> Encode(ReadOnlySpan<byte> bytes) => Encode(bytes, true, false);
    }
}